...
- **`frontend/`** — Angular 17 (standalone components, signals). Assistant chat, Today
  dashboard, tasks, notes, a six-month growth tracker, a fitness/diet tracker, news, and
  settings.
...
## Growth, fitness, and news

Three more tabs, entirely UI-driven (the agent doesn't touch these — they're not exposed as
...
- **Fitness & Diet** — a Push/Pull/Legs training split with a paired abs program, plus the
  diet plan, meal tables, and supplement schedule. Content lives in `fitness-data.ts`;
  daily workout/diet adherence is checked off and rolled into a 7-day percentage.
- **News** — on-demand research cards (global and local) that hand a search prompt to the
  chat assistant rather than polling any feed of their own.

Progress checkboxes persist under `assistant-roadmap-v1` and `assistant-fitness-v1`.
